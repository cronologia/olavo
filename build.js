...
const UI = {
  en: {
    ...
    dcTriadOne: 'one act',
    dcPxGap: 'displaced, and the gap is the defect',
    dcInvAlt: 'Three relations, each reversed end for end',
    dcStepsNote: 'no reasoning below a landing once reached',
    dcSplitOpen: 'and the next case is not settled',
    phReadingsHeading: 'How he read him',
    phReadingsNote: 'Reception, not philosophy: each entry records how Olavo de Carvalho characterised this philosopher, attributed and dated to the 2002 course. The project does not adjudicate who is right. The recordings are machine-transcribed and unchecked against audio, so everything here is paraphrase and nothing is quoted.',
  ...
  es: {
    ...
    dcPxGap: 'desplazados, y la distancia es el defecto',
    dcInvAlt: 'Tres relaciones, cada una invertida de cabo a rabo',
    dcStepsNote: 'no se razona por debajo de un rellano ya alcanzado',
    dcSplitOpen: 'y el caso siguiente queda sin resolver',
    phReadingsHeading: 'Cómo lo leyó',
    phReadingsNote: 'Recepción, no filosofía: cada entrada registra cómo caracterizó Olavo de Carvalho a este filósofo, atribuido y fechado en el curso de 2002. El proyecto no arbitra quién tiene razón. Las grabaciones están transcritas por máquina y no se han cotejado con el audio, de modo que todo lo que sigue es paráfrasis y nada se cita textualmente.',
  ...
  pt: {
    ...
    dcPxGap: 'deslocados, e a distância é o defeito',
    dcInvAlt: 'Três relações, cada uma invertida de ponta a ponta',
    dcStepsNote: 'não se raciocina abaixo de um patamar já alcançado',
    dcSplitOpen: 'e o caso seguinte fica sem resolver',
    phReadingsHeading: 'Como ele o leu',
    phReadingsNote: 'Recepção, não filosofia: cada verbete registra como Olavo de Carvalho caracterizou este filósofo, atribuído e datado no curso de 2002. O projeto não arbitra quem tem razão. As gravações são transcritas por máquina e não foram cotejadas com o áudio, de modo que tudo o que segue é paráfrase e nada é citado textualmente.',
...
`}
${philosophersHtml}${doctrinesHtml}${worksHtml}${swimlanesHtml}${placesMapHtml}${catalogueHtml}${tierMapHtml}${lineageHtml}${branchTimelineHtml}${numbersChartHtml}    <section id="figures">
...
function main() {
  ...
  for (const lang of LOCALES) {
    ...
    if (localizedPh) {
      ...
      // main page (disclaimerFor); philosophers.js prints `ui.disclaimer`.
