...
|---|---|---|
| **Husserl** | `Russel`, `russer`, `Russerl` | Reads as Bertrand Russell. In the Aristotle session the surrounding exchange is about phenomenology, and the same file attaches the name to *The Origin of Geometry* — Husserl's text. Taking the caption at face value would invert his stated 20th-century canon (Lavelle, Husserl, then Zubiri/Voegelin/Lonergan) into its opposite. Measured over the twelve 2002-course files: **0 canonical, 9 variant** — the correct spelling never appears. |
| **Lavelle** | `lavel`, `o lavel`, `Lavel`, `Lavell` | Cost a real finding. A word-boundary count of the correct spelling returns **0** across the twelve 2002-course files against 3 variant hits; the one passage it hides is his twentieth-century canon with Lavelle at its head. Any mention map over this corpus has to be run against the ASR spellings, not the true ones. |
| **Parmênides** | `parmes` | 15 occurrences against 2 for the correct spelling in the 2002 course — the mangling is the majority form, and the philosopher is the whole subject of the session that discusses his poem as myth. |
| **Thales** | `Tales` | Both spellings are used, 20 and 6, so either alone under-reports by a fifth to three quarters. Not an ASR error so much as an orthographic split, but the effect on a count is identical. |
...
| **Husserl** | `mundo rosto`, `Edmond Russell`, `o russo` | The same Husserl trap as above, in a third disguise. Found via the aphorism — no embryology of triangles, no trigonometry of lions. **`o russo` is a false friend and must be read in context**: it means "the Russian", and in the 2011 True Outspeak episodes it refers to Aleksandr Dugin, not Husserl. It is a true Husserl hit in the 2002 course (hef-007, the twentieth-century-canon passage) and a true Dugin hit five collections over. Never count it without reading the sentence. |
